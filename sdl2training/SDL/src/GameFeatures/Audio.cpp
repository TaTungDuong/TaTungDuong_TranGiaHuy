...
bool audioManager::loadMultiFile(std::vector<Mix_Chunk*>& target, std::string name, std::string format, int total)
{
	...
	{
		std::string path = "assets-main/audio/sfx/[audio]/" + name + " (" + std::to_string(i + 1) + ")." + format;
...
{
	//load music
	menu_bgm = Mix_LoadMUS("assets-main/audio/bgm/blue.mp3");
	if (menu_bgm == NULL)
	{
...
//	music = Mix_LoadMUS("assets-main/audio/sfx/[audio]/background music.mp3");
	main_bgm = Mix_LoadMUS("assets-main/audio/bgm/silence.mp3");
	if (main_bgm == NULL)
	{
		printf("Failed to load background music! SDL_mixer Error: %s\n", Mix_GetError());
		exit(1);
	...
	//load sound effects
	ambient = Mix_LoadWAV("assets-main/audio/sfx/[audio]/background ambient.mp3");
	if (ambient == NULL)
	{
	...
	radio = Mix_LoadWAV("assets-main/audio/sfx/[audio]/radio beep.mp3");
	if (radio == NULL)
	{
	...
	gunEmpty = Mix_LoadWAV("assets-main/audio/sfx/[audio]/gun empty.wav");
	if (gunEmpty == NULL)
	{
	...
	reload = Mix_LoadWAV("assets-main/audio/sfx/[audio]/reload.wav");
	if (reload == NULL)
	{
	...
	swapWeapon = Mix_LoadWAV("assets-main/audio/sfx/[audio]/swap weapon.mp3");
	if (swapWeapon == NULL)
	{
	...
	walk = Mix_LoadWAV("assets-main/audio/sfx/[audio]/walk.mp3");
	if (walk == NULL)
	{
	...
	hitTree = Mix_LoadWAV("assets-main/audio/sfx/[audio]/hit tree.mp3");
	if (walk == NULL)
	{
	...
	collectObject = Mix_LoadWAV("assets-main/audio/sfx/[audio]/collect object.mp3");
	if (walk == NULL)
	{
		printf("Failed to load collect object effect! SDL_mixer Error: %s\n", Mix_GetError());
		exit(1);
	}
	drinkMilk = Mix_LoadWAV("assets-main/audio/sfx/effects/drink.mp3");
	if (walk == NULL)
	{
		printf("Failed to load collect object effect! SDL_mixer Error: %s\n", Mix_GetError());
		exit(1);
	...
	gameLose = Mix_LoadWAV("assets-main/audio/sfx/[audio]/game lose.wav");
	if (walk == NULL)
	{
	...
	gameWin = Mix_LoadWAV("assets-main/audio/sfx/[audio]/game win.wav");
	if (walk == NULL)
	{
...
void audioManager::playDrinkMilk()
{
	setSoundEffect(-1, drinkMilk);
}
